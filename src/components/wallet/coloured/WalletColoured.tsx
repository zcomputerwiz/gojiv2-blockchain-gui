...
import { AlertDialog, Card, Flex } from '@goji/core';
      ...
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {moji_to_colouredcoin_string(props.balance)} {cc_unit}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
function BalanceCard(props: BalanceCardProps) {
  ...
  const balance_ptotal_goji = moji_to_colouredcoin_string(balance_ptotal);
  const balance_pending_goji = moji_to_colouredcoin_string(balance_pending);
  const balance_change_goji = moji_to_colouredcoin_string(balance_change);
  const acc_content =
    balancebox_1 +
    balancebox_2 +
    balancebox_ptotal +
...
function SendCard(props: SendCardProps) {
  ...
  function send() {
    ...
    const amount = colouredcoin_to_moji(amount_input.value);
    const fee = colouredcoin_to_moji(fee_input.value);

    if (address.includes('goji_addr') || address.includes('colour_desc')) {
      dispatch(
        openDialog(
          <AlertDialog>
